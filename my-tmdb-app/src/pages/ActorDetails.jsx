...
import { useParams, useNavigate } from "react-router-dom"; // 👈 تم إضافة useNavigate
import { useQuery } from "@tanstack/react-query";
import { fetchFromApi } from "../api/tmdb";
import { useActorMovieCredits } from "../hooks/useActorMovieCredits"; // 👈 استيراد الهوك الجديد
import MovieCard from "../components/MovieCard"; // 👈 لاستخدام كرت الفيلم
...
export default function ActorDetails() {
  ...
  // 2. جلب أفلام الممثل (Credits)
  const { data: credits, isLoading: isLoadingCredits } = useActorMovieCredits(id);
  ...
  if (isLoadingDetails) return <p className="p-6 text-white">Loading actor details...</p>;
  
  if (!actor) return <p className="p-6 text-red-500">Actor not found.</p>;
  
  // تجهيز قائمة الأفلام:
  const moviesCast = credits?.cast
  ...
  return (
    <div className="p-6 text-white">
      ...
      {/* 3. قائمة الأفلام التي شارك فيها (Known For) */}
      {isLoadingCredits && <p className="mt-10 text-gray-400">Loading filmography...</p>}
